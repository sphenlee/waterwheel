...
use jsonwebtoken::{DecodingKey, EncodingKey, Header, Algorithm, Validation, TokenData};
use std::fs;
...
pub fn load_rsa_keys() -> Result<()> {
    ...
    RSA_PUBLIC_KEY.set(DecodingKey::from_rsa_pem(&pub_key)?.into_static()).expect("public key already set??");

    let priv_key_file = std::env::var("WATERWHEEL_PRIVATE_KEY")?;
    let priv_key = fs::read(priv_key_file)?;
    RSA_PRIVATE_KEY.set(EncodingKey::from_rsa_pem(&priv_key)?).expect("private key already set??");
...
pub fn generate_jwt(task_id: &str) -> Result<String> {
    ...
    let claims = Claims {
        ...
        exp: (SystemTime::now() + Duration::from_secs(5 * 60)).duration_since(SystemTime::UNIX_EPOCH)?.as_secs()
    };
...
pub fn validate_jtw(jwt: &str) -> Result<String> {
    ...
    let token: TokenData<Claims> = jsonwebtoken::decode(jwt, RSA_PUBLIC_KEY.get().unwrap(), &validation)?;
